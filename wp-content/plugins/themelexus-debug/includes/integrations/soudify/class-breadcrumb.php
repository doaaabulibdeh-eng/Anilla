...
class Soudify_breadcrumb {
	...
	private function __construct() {
		...
		add_filter('soudify_page_title', [$this, 'hide_title']);
	...
	public function breadcrumb_register() {
		$labels = [
			'name'               => esc_html__('Breadcrumbs', 'soudify'),
			'singular_name'      => esc_html__('Breadcrumb', 'soudify'),
			'menu_name'          => esc_html__('Breadcrumbs', 'soudify'),
			'name_admin_bar'     => esc_html__('Breadcrumb', 'soudify'),
			'add_new'            => esc_html__('Add New', 'soudify'),
			'add_new_item'       => esc_html__('Add New Breadcrumb', 'soudify'),
			'new_item'           => esc_html__('New Header Footer & Blocks Template', 'soudify'),
			'edit_item'          => esc_html__('Edit Header Footer & Blocks Template', 'soudify'),
			'view_item'          => esc_html__('View Header Footer & Blocks Template', 'soudify'),
			'all_items'          => esc_html__('All Breadcrumb', 'soudify'),
			'search_items'       => esc_html__('Search Header Footer & Blocks Templates', 'soudify'),
			'parent_item_colon'  => esc_html__('Parent Header Footer & Blocks Templates:', 'soudify'),
			'not_found'          => esc_html__('No Header Footer & Blocks Templates found.', 'soudify'),
			'not_found_in_trash' => esc_html__('No Header Footer & Blocks Templates found in Trash.', 'soudify'),
		];
		...
		register_post_type('soudify-breadcrumb', $args);
	}
	...
	public function register_admin_menu() {
		add_submenu_page(
			'themes.php',
			esc_html__('Breadcrumbs', 'soudify'),
			esc_html__('Breadcrumbs', 'soudify'),
			'edit_pages',
			'edit.php?post_type=soudify-breadcrumb'
		);
	}
	...
	function ehf_register_metabox() {
		add_meta_box(
			'ehf-meta-box',
			esc_html__('Breadcrumb Options', 'soudify'),
			[
				$this,
				'efh_metabox_render',
			],
		...
		<table class="hfe-options-table widefat">
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e('Display On', 'soudify'); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
					   title="<?php echo esc_attr__('Add locations for where this template should appear.', 'soudify'); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					...
					HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
						...
						[
							'title'          => esc_html__('Display Rules', 'soudify'),
							'value'          => '[{"type":"basic-global","specific":null}]',
							'tags'           => 'site,enable,target,pages',
							'rule_type'      => 'display',
							'add_rule_label' => esc_html__('Add Display Rule', 'soudify'),
						],
						$include_locations
					);
					?>
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e('Do Not Display On', 'soudify'); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
					   title="<?php echo esc_attr__('Add locations for where this template should not appear.', 'soudify'); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					<?php
					HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
						...
						[
							'title'          => esc_html__('Exclude On', 'soudify'),
							'value'          => '[]',
							'tags'           => 'site,enable,target,pages',
							'add_rule_label' => esc_html__('Add Exclusion Rule', 'soudify'),
							'rule_type'      => 'exclude',
						],
						$exclude_locations
					);
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e('User Roles', 'soudify'); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help" title="<?php echo esc_attr__('Display custom template based on user role.', 'soudify'); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					<?php
					HFE\Lib\Astra_Target_Rules_Fields::target_user_role_settings_field(
						...
						[
							'title'          => esc_html__('Users', 'soudify'),
							'value'          => '[]',
							'tags'           => 'site,enable,target,pages',
							'add_rule_label' => esc_html__('Add User Rule', 'soudify'),
						],
						$users
					);
					?>
	...
	public static function get_template_id() {
		...
		$hfe_templates = Astra_Target_Rules_Fields::get_instance()->get_posts_by_conditions('soudify-breadcrumb', $option);
	...
	public function block_template_frontend() {
		if (is_singular('soudify-breadcrumb') && !current_user_can('edit_posts')) {
	...
	function load_canvas_template($single_template) {
		...
		if ('soudify-breadcrumb' == $post->post_type) {
