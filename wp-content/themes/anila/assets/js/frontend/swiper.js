(function ($) {
    ...
    const upsellsSwiper = $('.upsells .anila-theme-swiper');
    const relatedSwiper = $('.related .anila-theme-swiper');
    const crossSwiper = $('.cross-sells .anila-theme-swiper');
    if (crossSwiper.length > 0) {
        const ElementSettings = crossSwiper.data('settings');
        new Swiper('.cross-sells .anila-swiper', getSwiperSettings(ElementSettings));
    }
    if (upsellsSwiper.length > 0) {
        const ElementSettings = upsellsSwiper.data('settings');
        new Swiper('.upsells .anila-swiper', getSwiperSettings(ElementSettings));
    }
    if (relatedSwiper.length > 0) {
        const ElementSettings = relatedSwiper.data('settings');
        new Swiper('.related .anila-swiper', getSwiperSettings(ElementSettings));
    }
