( function($) {
	var Anila_Modal_Menu_Item = Backbone.View.extend({

		initialize: function( data ) {
			this.model = new Backbone.Model( data );
			this.template = '<span class="anila-edit-menu"><i class="fa fa-pencil"></i> Mega</span>';
		...
		_openModalHandler: function() {
			window.anila_menu_modal = new Anila_Modal( this.model );
		}

	});

	// Backbone Modal View
	var Anila_Modal = Backbone.View.extend({
		...
		initialize: function( model ) {
			...
			this.events = {
				...
				'click .anila-modal-overlay' : '_closeHandler',
		...
		render: function() {
			var that = this;
			this.template = $( '#tpl-anila-menu-item-modal' ).html();
			...
			data.action = 'anila_load_menu_data';
			data.nonce = anila_memgamnu_params.nonces.load_menu_data;
			...
			if ( this.model.get( 'is_loading' ) === true ) {
				$.ajax({
					type: 'POST',
					url: anila_memgamnu_params.ajaxurl,
					data: data,
					beforeSend: function() {
		...
		_submitEditMenuForm: function( e ) {
			...
			data.push({
				name: 'action',
				value: 'anila_update_menu_item_data'
			});
			...
			$.ajax({
				...
				data: data,
				beforeSend: function() {
					form.find( '.anila-modal-footer button' ).attr( 'disabled', true );
					form.find( 'button[type="submit"]' ).prepend( '<i class="fa fa-spin fa-spinner"></i> ' );
				}
			}).always( function() {
				form.find( '.anila-modal-footer button' ).attr( 'disabled', false );
			} ).done( function( res ) {
				if ( res.status === true ) {
					that.model.destroy();
				}
	...
	$( document ).on( 'mouseenter', '#menu-to-edit .menu-item-handle', function(e) {
		...
		menu_title.append( new Anila_Modal_Menu_Item({
			menu_id: menu_id,
			is_loading: true,
			istop:istop?1:0
		}).render().el );
		...
		var menu_item = $( this );
		var menu_title = menu_item.find( '.item-title' );
		menu_title.find( '.anila-edit-menu' ).remove();
	} );
} )(jQuery);
