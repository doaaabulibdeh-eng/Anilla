 ...
 * @extends WC_Widget
 */
class Anila_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'anila');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'anila');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'anila'),
				'label' => esc_html__('Title', 'anila'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'anila'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'anila'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'anila'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'anila'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'anila'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'anila'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'anila'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Anila_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'anila');
			...
			echo '<ul class="product-brands" id="anila-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Anila_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'anila'),
		'singular_name'              => esc_html__('Brands', 'anila'),
		'menu_name'                  => esc_html__('Brands', 'anila'),
		'all_items'                  => esc_html__('All Brands', 'anila'),
		'parent_item'                => esc_html__('Parent Brand', 'anila'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'anila'),
		'new_item_name'              => esc_html__('New Brand Name', 'anila'),
		'add_new_item'               => esc_html__('Add New Brands', 'anila'),
		'edit_item'                  => esc_html__('Edit Brand', 'anila'),
		'update_item'                => esc_html__('Update Brand', 'anila'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'anila'),
		'search_items'               => esc_html__('Search Brands', 'anila'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'anila'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'anila'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
