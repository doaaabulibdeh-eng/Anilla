...
class Anila_breadcrumb {
	...
	private function __construct() {
		...
		add_filter('anila_page_title', [$this, 'hide_title']);
	...
	public function breadcrumb_register() {
		$labels = [
			'name'               => esc_html__('Breadcrumbs', 'anila'),
			'singular_name'      => esc_html__('Breadcrumb', 'anila'),
			'menu_name'          => esc_html__('Breadcrumbs', 'anila'),
			'name_admin_bar'     => esc_html__('Breadcrumb', 'anila'),
			'add_new'            => esc_html__('Add New', 'anila'),
			'add_new_item'       => esc_html__('Add New Breadcrumb', 'anila'),
			'new_item'           => esc_html__('New Header Footer & Blocks Template', 'anila'),
			'edit_item'          => esc_html__('Edit Header Footer & Blocks Template', 'anila'),
			'view_item'          => esc_html__('View Header Footer & Blocks Template', 'anila'),
			'all_items'          => esc_html__('All Breadcrumb', 'anila'),
			'search_items'       => esc_html__('Search Header Footer & Blocks Templates', 'anila'),
			'parent_item_colon'  => esc_html__('Parent Header Footer & Blocks Templates:', 'anila'),
			'not_found'          => esc_html__('No Header Footer & Blocks Templates found.', 'anila'),
			'not_found_in_trash' => esc_html__('No Header Footer & Blocks Templates found in Trash.', 'anila'),
		];
		...
		register_post_type('anila-breadcrumb', $args);
	}
	...
	public function register_admin_menu() {
		add_submenu_page(
			'themes.php',
			esc_html__('Breadcrumbs', 'anila'),
			esc_html__('Breadcrumbs', 'anila'),
			'edit_pages',
			'edit.php?post_type=anila-breadcrumb'
		);
	}
	...
	function ehf_register_metabox() {
		add_meta_box(
			'ehf-meta-box',
			esc_html__('Breadcrumb Options', 'anila'),
			[
				$this,
				'efh_metabox_render',
			],
		...
		<table class="hfe-options-table widefat">
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e('Display On', 'anila'); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
					   title="<?php echo esc_attr__('Add locations for where this template should appear.', 'anila'); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					...
					HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
						...
						[
							'title'          => esc_html__('Display Rules', 'anila'),
							'value'          => '[{"type":"basic-global","specific":null}]',
							'tags'           => 'site,enable,target,pages',
							'rule_type'      => 'display',
							'add_rule_label' => esc_html__('Add Display Rule', 'anila'),
						],
						$include_locations
					);
					?>
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e('Do Not Display On', 'anila'); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
					   title="<?php echo esc_attr__('Add locations for where this template should not appear.', 'anila'); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					<?php
					HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
						...
						[
							'title'          => esc_html__('Exclude On', 'anila'),
							'value'          => '[]',
							'tags'           => 'site,enable,target,pages',
							'add_rule_label' => esc_html__('Add Exclusion Rule', 'anila'),
							'rule_type'      => 'exclude',
						],
						$exclude_locations
					);
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e('User Roles', 'anila'); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help" title="<?php echo esc_attr__('Display custom template based on user role.', 'anila'); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					<?php
					HFE\Lib\Astra_Target_Rules_Fields::target_user_role_settings_field(
						...
						[
							'title'          => esc_html__('Users', 'anila'),
							'value'          => '[]',
							'tags'           => 'site,enable,target,pages',
							'add_rule_label' => esc_html__('Add User Rule', 'anila'),
						],
						$users
					);
					?>
	...
	public static function get_template_id() {
		...
		$hfe_templates = Astra_Target_Rules_Fields::get_instance()->get_posts_by_conditions('anila-breadcrumb', $option);
	...
	public function block_template_frontend() {
		if (is_singular('anila-breadcrumb') && !current_user_can('edit_posts')) {
	...
	function load_canvas_template($single_template) {
		...
		if ('anila-breadcrumb' == $post->post_type) {
