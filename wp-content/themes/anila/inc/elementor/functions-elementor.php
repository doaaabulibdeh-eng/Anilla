...
if (!function_exists('anila_elementor_parse_text_editor')) {
    function anila_elementor_parse_text_editor($content, $obj) {
...
if (!function_exists('anila_elementor_get_strftime')) {
    function anila_elementor_get_strftime($instance, $obj) {
        $string = '';
...
if (!function_exists('anila_elementor_breakpoints')) {
    function anila_elementor_breakpoints() {
        ...
        wp_add_inline_style('anila-style', $var);
    }
}

function anila_get_hotspots() {
    global $post;

    $options[''] = esc_html__('Select Hotspots', 'anila');
...
function anila_get_file_contents($path) {
    if (is_file($path)) {
...
function anila_get_icon_svg($path, $color = '', $width = '') {
    $content = anila_get_file_contents($path);
    if ($content) {
        $re = '/<svg(([^\n]*\n)+)<\/svg>/';
